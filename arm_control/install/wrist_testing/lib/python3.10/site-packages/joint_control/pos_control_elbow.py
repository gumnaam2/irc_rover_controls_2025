...
class PIDController(Node):
    def __init__(self):
        ...
        # Parameters
        self.declare_parameter('velocity_threshold', 10)  # Threshold for switching to position hold
        ...
        # Publisher
        self.control_pub = self.create_publisher(ArmEndMotion, 'arm_commands_pid_elbow', 10)
        ...
        self.get_logger().info('PID Angle Controller Node started:Elbow')

    def cmd_vel_callback(self, msg):
        """
        Callback for velocity command input.
        """
        self.velocity_command = msg.speed[1]
        ...
        if abs(self.velocity_command) < self.velocity_threshold and self.is_setpoint is False:
            ...
            if self.setpoint!=self.current_position:
                self.get_logger().info(f'Setpoint updated:{self.current_position}:Elbow')
            self.setpoint = self.current_position  # Hold current position

            if abs(self.current_velocity)>0:
                if not self.control_active:
                    self.get_logger().info('Activating position hold:Elbow')
            ...
            if self.control_active:
                self.get_logger().info('Deactivating position hold:Elbow')
                self.control_active = False
    ...
    def encoder_callback(self, msg):
        ...
        """
        self.current_position = msg.arm_node1[0]
        self.current_velocity = msg.arm_node1[1]
...
def main(args=None):
    ...
    try:
        ...
        pid_node.get_logger().info('Shutting down PID Angle Controller Node:Elbow')
    finally:
